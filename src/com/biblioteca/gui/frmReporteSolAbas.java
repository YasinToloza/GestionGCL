...
public class frmReporteSolAbas extends JFrame implements ActionListener, ItemListener {
	...
	private JLabel lblReporteDeSolicitud;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					frmReporteSolAbas frame = new frmReporteSolAbas();
	 ...
	 * Create the frame.
	 */
	public frmReporteSolAbas() {
		setResizable(false);
		setTitle("Reporte Solicitud Abastecimiento");
		...
		btnReportar.setIcon(new ImageIcon(frmReporteSolAbas.class.getResource("/iconos/report.png")));
		...
		btnCerrar.setIcon(new ImageIcon(frmReporteSolAbas.class.getResource("/iconos/maintenance.png")));
		...
		btnBuscar.setIcon(new ImageIcon(frmReporteSolAbas.class.getResource("/iconos/search (1).png")));
		...
		rbtnTodo.setBounds(290, 18, 89, 23);
		...
		tblSolicitud.setModel(new DefaultTableModel(
			...
			new String[] {
				"C\u00F3digo", "Descripcion", "Fecha", "Estado", "Tipo Solicitud", "Cod. Empleado"
			}
		));
		...
		lblReporteDeSolicitud = new JLabel("REPORTE DE ABASTECIMIENTO");
		lblReporteDeSolicitud.setForeground(Color.WHITE);
		lblReporteDeSolicitud.setFont(new Font("Tahoma", Font.BOLD, 23));
		panel_2.add(lblReporteDeSolicitud);
	...
	protected void actionPerformedBtnReportar(ActionEvent e) {
		...
		String nombre;
		nombre="ReporteSolAbastecimiento.jasper";
	...
	protected void actionPerformedBtnBuscar(ActionEvent e) {
		...
		sep=edi.split("-");
		info=AbastecimientoDAO.listarSolicitudesConsulta(1,sep[0]);		
		}
		else if (rbtnCodigo.isSelected()) {
			info=AbastecimientoDAO.listarSolicitudesConsulta(2, txtBuscar.getText());
		}
		else if (rbtnFecha.isSelected()) {
			info=AbastecimientoDAO.listarSolicitudesConsulta(3, txtBuscar.getText());
		}
		else if (rbtnTodo.isSelected()) {
			info=AbastecimientoDAO.listarSolicitudesConsulta(4, "");
		}
		listarTodo(info);
	}
	...
	void listarTodo(ArrayList<Abastecimiento> data){
